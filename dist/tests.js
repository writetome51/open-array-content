...
if (copy.className && copy.className === 'PublicArrayContent')
    console.log('test 4A passed');
else
    console.log('test 4A FAILED');
// Test 4B: make sure copy has matching .data property:
if (arrays_match_1.arraysMatch(content.data, copy.data))
...
if (arrays_match_1.arraysMatch(content.data, copy.data))
    console.log('test 4C FAILED'); // they should not match.
else
...
if (content.data[0] === 'ccc' && content.data[1] === 'ddd')
...
var allData = [];
content.forEach(function (item, index, theArr) {
...
if (arrays_match_1.arraysMatch(content.data, [0, 1, 2, 3]))
    console.log('test 27A passed');
else
    console.log('test 27A FAILED');
// Test 28: make sure .set() changes value of content.data without breaking memory reference:
var dataCopy = content.data;
content.set([6, 7, 8, 9]);
if (arrays_match_1.arraysMatch(content.data, [6, 7, 8, 9]) && arrays_match_1.arraysMatch(content.data, dataCopy))
...
if (arrays_match_1.arraysMatch(content.data, [[0, 1], [8, 9], [2, 3], [4, 5], [6, 7]]))
